...
const chickenItems = food_list.filter((item) => {
  const searchText = `${item.name} ${item.description}`.toLowerCase();
  return searchText.includes('chicken');
});

const Menu2 = ({ cartItems, onAddToCart, onRemoveFromCart, onOrderItem, onBack }) => {
  return (
    <section className="py-12 sm:py-16">
      ...
      <div className="mb-10 space-y-3">
        ...
        <h2 className="text-3xl font-bold sm:text-4xl">Tasty chicken favorites</h2>
        <p className="max-w-2xl text-sm leading-7 text-slate-300 sm:text-base">
          Explore crispy, grilled, spicy, and creamy chicken dishes picked from our
          most popular items.
        </p>
      </div>
